...
def origin_plot_save(dataset, aug_type, config, dataset_type):
    dir = os.path.join("encoded_images", config.dataset, dataset_type, aug_type)
    os.makedirs(dir, exist_ok=True)
    if len(os.listdir(dir)) == len(dataset):
        return
    else:
    ...
    for i, input_data in enumerate(dataset):
        plt.close()
        plt.plot(input_data)
        plt.savefig(os.path.join(dir,f"{i}.png"))


def encoding_image1(input_dataset, input_y, random_int, aug_type, config, dataset_type):
    dir = os.path.join("encoded_images", config.dataset, dataset_type, aug_type)
    os.makedirs(dir, exist_ok=True)
    ...
    for idx, i in enumerate(pbar):
        label = input_y[idx:idx + 1]
        if f"{i}_{label}.png" in img_list:
            continue
        input_data = input_dataset[idx:idx + 1].numpy()
        input_data = [float(item) for item in input_data[0]]

        #initial_plot(config)
        plt.close()
        plt.plot(input_data)
        plt.savefig(os.path.join(dir, f"{i}_{label}.png"))
...
def encoding_image(input_dataset, random_int, aug_type, config, dataset_type):
    ...
    for idx, i in enumerate(pbar):
        ...
        #initial_plot(config)
        heatmap = cv2.resize(image[0], (224, 224))
        heatmap = np.array(heatmap*255, dtype=np.uint8)
        heatmap = cv2.applyColorMap(heatmap, cv2.COLORMAP_JET)
        cv2.imwrite(os.path.join(dir,f"{i}.png"), heatmap)
        time.sleep(0.01)
        #heatmap = cv2.normalize(heatmap, None, 0, 255, norm_type=cv2.NORM_MINMAX,
        #                            dtype=cv2.CV_8U)
    pbar.close()
