...
def DataTransform(sample, output, random_int, config, dataset_type):
    weak_aug = scaling(sample, config.augmentation.jitter_scale_ratio)
    strong_aug = jitter(permutation(sample, max_segments=config.augmentation.max_seg), config.augmentation.jitter_ratio)
    origin_plot_save(sample,output, random_int, config.datafolder_name+"_origin", config, dataset_type)
    encoding_image(weak_aug,output, random_int, config.datafolder_name+"_weak", config, dataset_type)
    encoding_image(strong_aug, output, random_int, config.datafolder_name + "_strong", config, dataset_type)
...
def origin_plot_save(input_dataset, output_dataset, random_int, aug_type, config, dataset_type):
    ...
    for idx, i in enumerate(pbar):
        if f"{i}.png" in img_list:
            continue
        plt.close()
        input_plot =input_dataset[idx]
        plt.plot(input_plot)
        label = output_dataset[idx]
        plt.savefig(os.path.join(dir, f"{i}_{label}.png"), bbox_inches="tight", pad_inches=0)
        plt.close()

def encoding_image(input_dataset,output_dataset, random_int, aug_type, config, dataset_type):
    dir = os.path.join("encoded_images", config.dataset, dataset_type, aug_type)
    os.makedirs(dir, exist_ok=True)
    img_list = os.listdir(dir)
    input_dataset = input_dataset.squeeze(1)
    ...
    def NormalizeData(data,vmin, vmax):
    ...
    for idx, i in enumerate(pbar):
        ...
        if flag_img < 3:
            ...
            image[0] = NormalizeData(image[0], vmin, vmax)
        elif flag_img == 3:
            coef, freqs = pywt.cwt(input_dataset[idx].numpy(), np.arange(1, 225), 'morl')
            image=[]
            image.append(coef)
            image[0] = NormalizeData(image[0], -1*np.max(abs(image[0])), np.max(abs(image[0])))
        #initial_plot(config)
        heatmap = cv2.resize(image[0], (224, 224), interpolation=cv2.INTER_NEAREST)
        heatmap = np.array(heatmap*255, dtype=np.uint8)
        heatmap = cv2.applyColorMap(heatmap, cv2.COLORMAP_JET)
        label = output_dataset[idx]
        cv2.imwrite(os.path.join(dir,f"{i}_{label}.png"), heatmap)
        time.sleep(0.01)
        #heatmap = cv2.normalize(heatmap, None, 0, 255, norm_type=cv2.NORM_MINMAX,
        #                            dtype=cv2.CV_8U)
    pbar.close()
