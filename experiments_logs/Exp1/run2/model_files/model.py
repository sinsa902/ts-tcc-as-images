...
class densenet_model(nn.Module):
    def __init__(self, configs):
        super().__init__()
        model = timm.create_model(configs.modelname, pretrained=configs.pretrain,
        ...
        self.project = nn.Conv2d(self.densenet_channel, configs.embed_dim, kernel_size=configs.patch_size,
...
class resnet_model(nn.Module):
    def __init__(self, configs):
        ...
        self.project = nn.Conv2d(self.resnet_channel, configs.embed_dim, kernel_size=configs.patch_size,
                                 stride=1, padding='same')
...
class make_patch_logits(nn.Module):
    def __init__(self, configs):
        ...
        self.conv_block1 = nn.Sequential(
            nn.Conv2d(configs.input_channels, 12, kernel_size=configs.kernel_size, padding=(configs.kernel_size // 2)),
            nn.BatchNorm2d(12),
            nn.ReLU(),
        ...
        self.conv_block2 = nn.Sequential(
            nn.Conv2d(12, 32, kernel_size=configs.kernel_size, padding=(configs.kernel_size // 2)),
            nn.BatchNorm2d(32),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2),
            # nn.Dropout2d(configs.dropout)
        ...
        self.project = nn.Conv2d(configs.final_out_channels, configs.embed_dim, kernel_size=configs.patch_size,
                                 stride=configs.patch_size)
        self.logits = nn.Sequential(
            nn.Linear(configs.features_len * configs.final_out_channels, configs.num_classes),
...
class base_Model_rev1(nn.Module):
    def __init__(self, configs):
        ...
        self.conv_block1 = nn.Sequential(
            nn.Conv2d(configs.input_channels, 32, kernel_size=configs.kernel_size,
        ...
        self.conv_block2 = nn.Sequential(
            nn.Conv2d(32, 64, kernel_size=configs.kernel_size,
                      stride=configs.stride, bias=False, padding=(configs.kernel_size // 2)),
            nn.BatchNorm2d(64),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2)
        ...
        self.conv_block3 = nn.Sequential(
            nn.Conv2d(64, configs.final_out_channels, kernel_size=configs.kernel_size, stride=1, bias=False,
